...
void usage(char const* programName) {
  std::cerr << "Usage: " << programName << " program.l1 [--gen-asm-only] output-file\n\n"
            << "This program compiles given L1 program. If the `--gen-asm-only` option is given, it will only generate the assembly code, otherwise it will also link the assembly code with the bootstrap code to produce an executable.";
}
...
int main(int argc, char* argv[]) {
  ...
  if (link) {
    ...
    // Compile the bootstrap code
    cmd = "gcc -m32 -c bootstrap.c -o bootstrap.o";
    std::cout << "Compiling the bootstrap code\nRunning GCC: " << cmd << std::endl;
    if (auto err = std::system(cmd.c_str())) {
      std::cerr << "GCC exited with error code " << err << std::endl;
      return 1;
    }
    ...
    cmdLine << "gcc -m32 bootstrap.o " << outputFileName << ".o -o " << outputFileName;
    cmd = cmdLine.str();
